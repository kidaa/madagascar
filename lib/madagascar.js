...
module.exports = function(conf) {
  ...
  return function(payload, done) {
    ...
    payload = _.defaults(payload, {
      base_url: conf.domains.baseUrl,
      authentication: {},
      batch: []
    });

    if (payload.batch.length > conf.batchMaxSize) {
    ...
    for (var i in payload.batch) {
      ...
      args.headers = _.defaults(args.headers || {}, conf.defaultHeaders);

      if (conf.domains.restrictTo.length > 0 &&
          conf.domains.restrictTo.indexOf(url.host()) < 0) {
        done({
